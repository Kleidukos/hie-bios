  ...
  Cradle
    ...
    , cradleOptsProg   = CradleAction
        ...
        , runGhcCmd = \args -> do
            ...
            realCabalProcess <- cabalProcess wdir "v2-exec" $ ["ghc", "-v0", "--"] ++ args
            readProcessWithCwd'
              realCabalProcess ""
        }
    }

cabalProcess :: FilePath -> String -> [String] -> IO CreateProcess
...
cabalGHCDir :: IO FilePath
cabalGHCDir = do
  result <- readProcessWithCwd "." "cabal" ["exec", "-v0", "--", "ghc", "--print-libdir"] ""
  case result of
    CradleFail err -> error (show err)
    CradleNone     -> undefined
    CradleSuccess r -> do
      let strippedPath = T.unpack $ T.stripEnd $ T.pack r
      if isWindows
      then pure $ strippedPath </>"../bin"
      else pure $ strippedPath </> "../../bin"
...
cabalAction workDir mc l fp = do
    realCabalProcess <- cabalProcess workDir "v2-repl" [fromMaybe (fixTargetPath fp) mc]
    (ex, output, stde, [(_, maybeArgs)]) <-
      readProcessWithOutputs [hie_bios_output] l workDir realCabalProcess
...
readProcessWithCwd' createdProcess stdin = do
  ...
  let cmdString = show $ cmdspec createdProcess
  case mResult of
